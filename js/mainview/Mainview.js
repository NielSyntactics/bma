var mainView = function () {
	...
	function init(params) {
		...
		Ext.Ajax.request({
			...
			, success: function (response) {
				...
				var schoolyear = result.schoolyear;

				console.log(result);

				schoolyear.forEach(sy => {
					var selected = (sy['isSet'] == 1) ? 'selected' : '';
					options += '<option ' + selected + ' value="' + sy['schoolYearID'] + '">' + sy['schoolYearStart'] + '-' + (parseInt(sy['schoolYearStart']) + 1) + '</option>';
				});
	...
	function extLoader(module, async, length, callback) {
		...
		if (module) {
			Ext.Loader.loadScript({
				...
				, onLoad: onLoad
				, onError: function () {
					console.log('here in extLoader error');
					loadModulesProgressWin.destroy(true);
	...
	function headerhtml() {
		...
		return '<table width="100%" height="70px" style="background-color:#FDE6CE; color: #512A03; ">' +
			...
			'<td><select id="setSchoolYear" class="x-form-field x-form-text" style="margin:0 5px;">' + options + '</select></td>' +
			'<td><button onclick="setSchoolYear()" class="x-btn x-unselectable x-column x-btn-default-small x-icon x-btn-icon x-btn-default-small-icon" style="color: #c59d86 !important; font-weight: bold; padding: 2px 10px;">Set School Year</button></td>' +
			'</tr>' +
			'</table>' +
			'</td>' +
			'<td valign="bottom">' +
					...
					, success: function (response, options) {
						...
						if (parseInt(response.responseText, 10) == 0) {
							...
							Ext.Ajax.request({
								...
								, noMask: true
								, success: function (response, options) {
									console.log('here in extLoader login');
	...
	function initializeMenu() {
		...
		var buttonLabel = ['Account Card', 'Admin', 'Report', 'Payments', 'Help'];
		var buttonID = ['btnHome', 'btnReports', 'btnAdmin', 'btnPayments', 'btnHelp'];
		var menuContainerID = ['menuHome', 'menuReports', 'menuAdmin', 'menuPayments', 'menuHelp'];
		var menuDivID = ['divHome', 'divReports', 'divAdmin', 'divPayments', 'divHelp'];
	...
	function showMenu(mtype) {
		...
		if (payments = Ext.getCmp('menuPayments'+ exts)) {
			payments.setVisible(mtype == 3);
		}

		if (admin = Ext.getCmp('' + exts)) {
			admin.setVisible(mtype == 4);
		}
		...
		if (helpBtn = Ext.getCmp('helpBtn' + exts)) {
			helpBtn.addCls(((mtype == 4) ? 'btn-mainMenu-Active' : 'btn-mainMenu'));
			helpBtn.removeCls(((mtype != 4) ? 'btn-mainMenu-Active' : 'btn-mainMenu'));
		}

		if (btnPayments = Ext.getCmp('btnPayments' + exts)) {
			btnPayments.addCls(((mtype == 3) ? 'btn-mainMenu-Active' : 'btn-mainMenu'));
			btnPayments.removeCls(((mtype != 3) ? 'btn-mainMenu-Active' : 'btn-mainMenu'));
		}

		Ext.getCmp('mainTab' + exts).setActiveTab('menu' + exts);
		console.log('showMenu', mtype);
	}

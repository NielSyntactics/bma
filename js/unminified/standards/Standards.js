var standards = function(){
	...
	function _createCombo( params ){
		...
		if( typeof params.type != 'undefined' ){
			...
			if( type == 'user' ){
				...
				params['id'] 		 = ( typeof params.id != 'undefined' )? params.id : 'precinctID' + params.module;
				params['fieldLabel'] = ( typeof params.fieldLabel != 'undefined' )? params.fieldLabel : 'Precinct #';
				params['hasQuick']	 = ( typeof params.hasQuick != 'undefined' )? params.hasQuick : true;
				
				if( Ext.ifUndefined( params.hasQuick, true ) ){
				...
				listeners['select'] = function( me, result ){
					if( me.getValue() == 0 ){
						me.reset();
						me.fireEvent( 'blur' );
						_precinctWin( params, me );
					}
				}
			}
			else if( type == 'barangay' ){
				params['id'] 		 = ( typeof params.id != 'undefined' )? params.id : 'barangayID' + params.module;
				params['fieldLabel'] = ( typeof params.fieldLabel != 'undefined' )? params.fieldLabel : 'Barangay';
				rank 			 	 = 1;
			}
			else if( type == 'city' ){
				params['id'] 		 = ( typeof params.id != 'undefined' )? params.id : 'cityMunicipalityID' + params.module;
				params['fieldLabel'] = ( typeof params.fieldLabel != 'undefined' )? params.fieldLabel : 'City/Municipality';
				rank 			 	 = 2;
				type				 = 'cityMunicipality';
			}
			else if( type == 'province' ){
				params['id'] 		 = ( typeof params.id != 'undefined' )? params.id : 'provinceID' + params.module;
				params['fieldLabel'] = ( typeof params.fieldLabel != 'undefined' )? params.fieldLabel : 'Province';
				rank 			 	 = 3;
			}
			else if( type == 'region' ){
				params['id'] 		 = ( typeof params.id != 'undefined' )? params.id : 'regionID' + params.module;
				params['fieldLabel'] = ( typeof params.fieldLabel != 'undefined' )? params.fieldLabel : 'Region';
				rank 			 	 = 4;
			}
			...
			if( rank != 0 ){
				...
				listeners['beforequery'] = function( me ){
					var addressFilters = ['barangayID', 'cityMunicipalityID', 'provinceID', 'regionID'];
					
					for( x in addressFilters ){
						params['store'].proxy.extraParams[addressFilters[x]] = null;
						if( filter = Ext.getCmp( addressFilters[x] + params.module ) ){
							if( filter.getValue() ){
								params['store'].proxy.extraParams[addressFilters[x]] = filter.getValue();
							}
						}
					}
				...
				listeners['select'] = function( me, value ){
					var address = ['cityMunicipalityID', 'provinceID', 'regionID'];
					var retData = value[0].data;
					var start	= address.indexOf( me.id.replace( params.module, '' ) );
					
					if( retData.name == '<font color=grey> Reset field </font>' ){
						if( retData ){
							me.reset();
							for( var x=start+1; x<address.length; x++ ){
								if( field = Ext.getCmp( address[x]+ '' + params.module ) ){
									if( field.getValue() ){
										field.reset();
									}
								}
						...
						function selectCallBack( field, x ){
							return function(){
								field.setValue( parseInt( retData['top'+x], 10 ) );
							}
						}
						...
						if( retData ){
							for( var x=start+1; x<address.length; x++ ){
								if( field = Ext.getCmp( address[x]+ '' + params.module ) ){
									if( !field.getValue() ){
										field.store.load({
											callback: selectCallBack( field, x )
										});
									}
								}
							}
			...
			,renderer: function( value, metaData, rec ){
				...
				if( typeof params.condition != 'undefined' ){
					...
					if( eval( params.condition ) ){
						return '<span class="glyphicon glyphicon-'+params.icon2+'" style="color:'+color+'" title="'+params.tooltip2+'"></span>';
					}
				}
	...
	*/
	...
	function _mainPanel( params ){
		...
		mainTab.setActiveTab( 'mainPanel'+params.module );
	...
	function _changeCls( params ){
		...
		for( var x=0; x<main.items.items.length; x++ ){
			if( params.scope.id == tbar[x].id ){
				...
				// Ext.getCmp('tbarCardPanel'+params.module).getLayout().setActiveItem( 1);
				// console.log(Ext.getCmp('tbarCardPanel'+params.module).getLayout());
			}
			else{
	...
	*/
	...
	function _goToForm( params ){
		...
		if( _changeCls({
			...
			if( btnExcel = params.module.getButton( 'excel' ) ){
				console.log(form.onEdit);
				console.log(main.config.canPrint);
				console.log(params.hasFormExcel);
		...
		for( var x=0; x<numberFilter; x++ ){
			...
			items.push(
				_createCombo({
					...
					,emptyText:		( typeof params.emptyText != 'undefined' )? params.emptyText : 'Search '+( hasSubFilter? params.subFilters[0].toLowerCase() : 'here' )+'...'
					,hideTrigger:	true
					,listeners:{
						select : function( me ){
							var store;
	...
	*/
	...
	function _listExcel( params ){
		...
		if( params.customListExcelHandler ){
			...
			Ext.Ajax.request({
				...
				,success: function(){
					window.open( standardRoute + "download/" + params.pageTitle + ' List' + '/' + path.substring( 0, index ) );
				}
			});
		}
	}
	...
	function _precinctWin( params ){
		...
		var store 	  = _createRemoteStore({
							...
							,url: standardRoute + 'getPrecinctHistory'
						});
		
		store.load();
		
		Ext.create( 'Ext.Window', {
			id:'precinctWindow'+winModule
			,title:'Add Bew Precinct Number'
			,autoWidth:true
			,autoHeight:true
			,modal:true
			,bodyPadding:'10px'
			...
			,items: [
				{
					xtype:'hiddenfield'
					,id:'precinctID'+winModule
				}
				,{
					xtype:'hiddenfield'
					,id:'dateModified'+winModule
				}
				,{
					xtype:'hiddenfield'
					,id:'onEdit'+winModule
					...
					,items:[
						{
							items:[
								_createTextField({
									id:'precinctName'+winModule
									,fieldLabel:'Precinct Number'
									,allowBlank:false
									,maxLength:250
								})
									...
									,items:[
										...
										,{
											...
											,handler:function(){
												...
												Ext.getCmp( 'onEdit'+winModule ).setValue( 0 );
												Ext.getCmp( 'dateModified'+winModule ).reset();
												Ext.getCmp( 'modify'+winModule ).reset();
												Ext.getCmp( 'precinctID'+winModule ).reset();
											}
										}
									]
								}
				...
				,_gridPanel({
					...
					,columns: [
						...
						,_createActionColumn({
							...
							,Func		: function( data ){
								Ext.Ajax.request({
									url:	 standardRoute+'retrievePrecinctData'
									,params:{
									...
									,success:function( response ){
										...
										Ext.getCmp( 'precinctID'+winModule ).setValue( view.precinctID );
										Ext.getCmp( 'precinctName'+winModule ).setValue( view.precinctName );
										Ext.getCmp( 'barangayID'+winModule ).store.load({
											callback:function(){
												Ext.getCmp( 'barangayID'+winModule ).setValue( parseInt( view.barangayID, 10 ) );
											}
										});
										
										Ext.getCmp( 'cityMunicipalityID'+winModule ).store.load({
											callback:function(){
												Ext.getCmp( 'cityMunicipalityID'+winModule ).setValue( parseInt( view.cityMunicipalityID, 10 ) );
											}
										});
										
										Ext.getCmp( 'provinceID'+winModule ).store.load({
											callback:function(){
												Ext.getCmp( 'provinceID'+winModule ).setValue( parseInt( view.provinceID, 10 ) );
											}
										});
										
										Ext.getCmp( 'regionID'+winModule ).store.load({
											callback:function(){
												Ext.getCmp( 'regionID'+winModule ).setValue( parseInt( view.regionID, 10 ) );
											}
										});
										
										Ext.getCmp( 'onEdit'+winModule ).setValue( 1 );
										Ext.getCmp( 'dateModified'+winModule ).setValue( view.dateModified );
									}
								});
							}
						})
						,_createActionColumn({
							...
							,Func	: function( data ){
								data.confirmDelete({
									url: standardRoute + 'deletePrecinctData'
									,params:{
										precinctID   : data.precinctID
									...
									,success:function( response ){
										...
										if( ret.match == 1 ){
											...
											if( Ext.getCmp( 'precinctID'+winModule ).getValue() == data.precinctID ){
		...
		function _savePrecinctForm(  ){
			var name = Ext.getCmp( 'precinctName' + winModule ).getValue();
			var form = Ext.getCmp( 'mainFormPanel' + winModule ).getForm();
			
			form.submit({
				url:	standardRoute+'savePrecinctForm'
				,params:{
					module 		: winModule
					,precinctID	: Ext.getCmp( 'precinctID' + winModule ).getValue()
					,onEdit		: Ext.getCmp( 'onEdit' + winModule ).getValue()
					,modify		: Ext.getCmp( 'modify' + winModule ).getValue()
					,dateModified: Ext.getCmp( 'dateModified' + winModule ).getValue()
				}
				,success:function( action, response ){
					...
					if( match == 1 ){
						_createMessageBox({
							msg : 'Precinct Number : ' + name + ' already exists.'
						});
					}
					else if( match == 2 ){
						_createMessageBox({
						...
						_createMessageBox({
							...
							,fn		: function( btn ){
								if( btn == 'yes' ){
									form.modify = true;
									Ext.getCmp( 'modify'+winModule ).setValue( 1 );
									_savePrecinctForm();
								}
							}
						});

var Activityguide = function(){
	...
	var baseurl, route, module, canDelete, _idmodule, editHiddenName, bool=true;
	var gFilterID ,gTableID , gDispValue, visibility = 1;
							...
							,{
								...
								[
									,standards.callFunction( '_createCombo' ,{
											...
											,listeners: {
												beforequery: function(  ){
													Ext.getCmp( "schoolYear" + module ).store.proxy.extraParams.filterClosed = 1
												}
												,select: function(combo, record, index) {
												
													var id =  Ext.getCmp( 'schoolYear_Activityguide').getValue();
													// }else{
														// id = combo.getValue();
													// }
									...
									,standards.callFunction( '_createCombo', {
										...
										,width			: 	430
										,listeners: {
											extraSelect: function(combo, record, index) {
														Ext.getCmp( 'activityClassification' + module ).setValue( parseInt( record[0].data.classificationID ) );
														Ext.getCmp( 'activityClassification' + module ).fireEvent( 'select' );
												// Ext.getCmp( 'activityClassification' + _module ).store.load( {
												// 	callback: function(){
												// 		Ext.getCmp( 'activityClassification' + _module ).setValue( record[0].data.classificationID );
												// 		Ext.getCmp( 'activityClassification' + _module ).fireEvent( 'select' );
												// 	}
												// } )
											}
										}
							...
							,{
								...
								[
									standards.callFunction( '_createCombo' ,{
											...
											,listeners: {
												select: function(combo, record, index) {
													if( Ext.getCmp( "activityClassification" + module ).getValue() == 2){
														standards.callFunction( 'setAllowBlankComponent' ,{
															component : Ext.getCmp( 'activityWrittenWorkType' + module )
															,allowBlank : false
														});
													}else{
														standards.callFunction( 'setAllowBlankComponent' ,{
													...
													Ext.getCmp( 'activityWrittenWorkType' + module ).allowBlank = ( Ext.getCmp( "activityClassification" + module ).getValue() == 3 ) ? false : true;
						...
						,listeners:{
							select:function( combo, record, index ){
								...
								// store.load({
									...
									// }
								// });
								gFilterID = this.getValue(); 
								gTableID = Ext.getCmp( 'filterType' + module).getValue();
								gDispValue = Ext.getCmp( 'filterTypeList' + module ).getDisplayValue();
								gridCallbackFilter();
							}
							,beforequery: function(){
			...
			,columns: [
				...
				// ,{
					...
					// ,summaryRenderer: function(value, summaryData, dataIndex) {
					   // if(!value) value = 0;
					   // Ext.getCmp('totalMembers'+module).setValue(Ext.util.Format.number( Ext.getCmp('gridList'+module).store.getCount(),'0,000'));
					// }
				// }
			]
		});
	...
	function _editRecord1( data ) {
		if( typeof Ext.getCmp( "schoolYear" + module ).store.proxy.extraParams.filterClosed != 'undefined' ){
			delete Ext.getCmp( "schoolYear" + module ).store.proxy.extraParams.filterClosed
		}
		module.getForm().retrieveData({
				...
				Ext.getCmp( 'schoolYear' + module ).store.load({
					callback: function( a,b,c ){
						...
						module.getForm().retrieveData({
							...
							,success: function( a,b,c ){
								if( isNaN( Ext.getCmp( 'activityWrittenWorkType' + _module ).getValue() ) ){
									Ext.getCmp( 'activityWrittenWorkType' + _module ).setValue( "" );
								}
								editHiddenName = "Modified " + a.activityCode + " " + Ext.getCmp('activityClassification' + module ).getDisplayValue() + " record";
								Ext.getCmp('studentGradeLevelHistoryID' + module ).setValue("")
								Ext.getCmp( 'gradeLevelID' + module ).setValue( "" );
								Ext.getCmp( 'studentGradeLevelHistoryID' + module ).setValue( a.gradeLevelDescription );
								Ext.getCmp( 'gradeLevelID' + module ).setValue( a.gradeLevelID );;
								Ext.getCmp( 'activityCode' + module ).setValue( actCode );
								Ext.getCmp( "saveButton" + module).setVisible(visibility)
							}
						});
					}
				});
	...
	function _editRecord( data ){
		console.warn( data )
		Ext.Ajax.request({
			url: route + 'retrieveActivityDatas'
			,params:{
				activityID 	: 	data.activityID
			}
		    ,success: function(response){
		        ...
		        var match = parseInt( ret.match );
				if( match == 11 ){
					standards.callFunction( '_createMessageBox', {
						msg : 'School year ' + ret.view + ' has been closed. You are no longer allowed to edit this activity.'
					});	
				}
				visibility = 0;
	...
	// console.log( data );
	var delActivity = "Deleted " + data.activityCode + " " + data.activityClassification
		data.confirmDelete({
			url: route + 'deleteActivity'
			,params:{
				activityID   : data.activityID
				,idmodule 	 : _idmodule
				,description : 	delActivity
			}
			,success:function( response ){
				var ret = Ext.decode( response.responseText );
				if( ret.match == 1 ){
	...
	// console.log( route );
		form.submit({
			...
			,success:function( action, response ){
				...
				if( match == 1 ){
					standards.callFunction( '_createMessageBox', {
						msg : 'Activity can no longer be edited since it already has a grade on the grading sheet.'
					});
				}
				else if( match == 2 ){
					standards.callFunction( '_createMessageBox', {
						msg : 'Activity can no longer be added since the grading sheet for this quarter is final.'
					});
				}
				else if( match == 3 ){
					standards.callFunction( '_createMessageBox', {
					...
					standards.callFunction( '_createMessageBox', {
						msg : 'This activity already exist.'
					});	
				}
				else if( match == 6 ){
					standards.callFunction( '_createMessageBox', {
						msg		: 'You have changed the activity\'s subject. This will remove all scores for this activity under the previously-selected subject.  Do you want to proceed? Yes/No'
						,action	: 'confirm'
						,fn		: function( btn ){
							if( btn == 'yes' ){
								form.modify = true;
					...
					standards.callFunction( '_createMessageBox', {
						msg : 'School year '+ Ext.getCmp( 'schoolYear' + module ).getDisplayValue() +' has been closed. You are no longer allowed to save this activity.'
					});	
				}
				else{
					standards.callFunction( '_createMessageBox', {
	...
	function gridCallbackFilter(){
			Ext.getCmp( 'gridHistory' + module ).store.currentPage = 1;
			Ext.getCmp( 'gridHistory' + module ).store.proxy.extraParams.filter = gFilterID;
			Ext.getCmp( 'gridHistory' + module ).store.proxy.extraParams.tableID = gTableID;
			Ext.getCmp( 'gridHistory' + module ).store.proxy.extraParams.dispValue = gDispValue;
		...
		Ext.getCmp( 'gridHistory' + module ).store.load({
			...
			// }
			callback: function(){
				if( Ext.getCmp( 'gridHistory' + module ).store.getCount() == 0 ){
					standards.callFunction( '_createMessageBox', {
						msg : 'No records found.'
					});
				}

...
var Accountcard = function () {
	...
	*/
				...
				, edit: function (me, grid, a, b) {
					...
					*/
					if (me.context.colIdx == 1) {
						if (record.get('_ref') == 'OR' || record.get('_ref') == 'TR' || record.get('_ref') == 'SI') {
						...
						if (
							...
							Ext.Ajax.request({
								...
								success: function (res) {
									...
									if (ret.fileName != '') {
										console.log('here');
										const downloadUrl = _baseurl + 'images/acknowledgement/' + ret.fileName;


										const link = document.createElement('a');
										link.href = downloadUrl;
										link.download = ret.fileName; // Optional: set a specific file name
	...
	function checkStudentAge(form, params) {

		var objectData = new Object()

		var gradeID = Ext.getCmp("gradeLevelID" + _module).getValue();
		var birthDate = Ext.getCmp("studentBirthday" + _module).getValue();

		objectData["gradeID"] = gradeID;
		objectData["birthDate"] = birthDate;

		Ext.Ajax.request({
			url: _route + "checkStudentAge",
			method: "POST",
			params: objectData,
			success: function (response) {
		...
		form.submit({
			...
			, success: function (action, response) {
				...
				switch (match) {
					case 0:
						...
						Ext.getCmp("searchSYStore" + _module).store.load({
							callback: function () {
								...
								Ext.getCmp("grdTransactions" + _module).setVisible(true)
								Ext.getCmp("searchSYStore" + _module).setValue(parseInt(resp.yearLevel, 10));
					...
					case 6:
						standards.callFunction('_createMessageBox', {
							msg: 'Student is Overage for this Grade Level.'
						});
						break;
					default:
						standards.callFunction('_createMessageBox', {
				...
				Ext.getCmp("searchSYStore" + _module).store.load(
					{
						...
						, callback: function () {
							...
							console.log("here it is", response.accountCardSchoolYear);
							Ext.getCmp('searchSYStore' + _module).fireEvent('select');

...
class Batchreceivables extends CI_Controller {
	...
	public function getSchoolYear(){
		...
		die(
			json_encode(
				array(
					'success' => true
					,'isSet' => $this->model->getMainSchoolYear( $params )
	...
	public function deleteRecord(){
		...
		$this->model->deleteReceivable( (int)$params['batchReceivableID'],(int)$params['studentID'] );

		$receivableCnt =$this->model->getReceivable( (int)$params['batchReceivableID']);
		/* delete records in header(batchreceivables table) */
		if($receivableCnt <= 0)$this->model->deleteRecord( $params );

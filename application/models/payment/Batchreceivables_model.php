...
class Batchreceivables_model extends CI_Model {
	...
	public function getSchoolYear( $params, $type = 1 ){
		if( $type == 1 ) $this->db->select( " CONCAT( schoolYearStart, ' - ', schoolYearStart + 1 ) as schoolYearStart, schoolYearID, isSet" );
	...
	function getMainSchoolYear()
	{
		return $this->db->where('isSet', 1)->get('schoolyear')->row()->schoolYearID;
	}
	...
	public function getHistory( $params ){
		...
		$this->db->order_by( 'a.batchReceivableDate desc' );
	...
	public function deleteReceivable( $batchReceivableID, $studentID ){
		$this->db->where( 'batchReceivableID', $batchReceivableID );
		if($studentID !== '')$this->db->where( 'studentID', $studentID);
	...
	public function getReceivable( $batchReceivableID ){
		return $this->db->query("
			select * from receivables where batchReceivableID = $batchReceivableID
		")->num_rows();
	}

...
class Accountcard_model extends CI_Model {
	...
	public function getGradeLevels ( $params = array() ){
		...
		$this->db->order_by('dateOfBirth', 'ASC');
	...
	public function saveAccTrans( $data ){ 
		...
		if ( $onEdit )
		{
			...
			$updateData = unsetParams($data, 'accountcard');
			if (!isset($data['movingUpTotalReceivable']) && !isset($data['nutritionTotalReceivable'])) {
				$this->db->where( 'accountCardID', $accountCardID );
				$this->db->update( 'accountcard', $updateData );
			}

			// update the receivables
			if (isset($data['cateringTotalReceivable'])) {
				$this->updateReceivablesForAdjustment($params['studentID'], $params['schoolYearID'], 1, $data['cateringTotalReceivable']);
			}

			if (isset($data['charityTotalReceivable'])) {
				$this->updateReceivablesForAdjustment($params['studentID'], $params['schoolYearID'], 2, $data['charityTotalReceivable']);
			}

			if (isset($data['extracurricularTotalReceivable'])) {
				$this->updateReceivablesForAdjustment($params['studentID'], $params['schoolYearID'], 3, $data['extracurricularTotalReceivable']);
			}

			if (isset($data['christmasTotalReceivable'])) {
				$this->updateReceivablesForAdjustment($params['studentID'], $params['schoolYearID'], 4, $data['christmasTotalReceivable']);
			}

			if (isset($data['familyDayTotalReceivable'])) {
				$this->updateReceivablesForAdjustment($params['studentID'], $params['schoolYearID'], 5, $data['familyDayTotalReceivable']);
			}

			if (isset($data['pictureTotalReceivable'])) {
				$this->updateReceivablesForAdjustment($params['studentID'], $params['schoolYearID'], 6, $data['pictureTotalReceivable']);
			}

			if (isset($data['gradFeeTotalReceivable'])) {
				$this->updateReceivablesForAdjustment($params['studentID'], $params['schoolYearID'], 7, $data['gradFeeTotalReceivable']);
			}

			if (isset($data['scoutingTotalReceivable'])) {
				$this->updateReceivablesForAdjustment($params['studentID'], $params['schoolYearID'], 8, $data['scoutingTotalReceivable']);
			}

			if (isset($data['othersTotalReceivable'])) {
				$this->updateReceivablesForAdjustment($params['studentID'], $params['schoolYearID'], 9, $data['othersTotalReceivable']);
			}

			if (isset($data['nutritionTotalReceivable'])) {
				$this->updateReceivablesForAdjustment($params['studentID'], $params['schoolYearID'], 10, $data['nutritionTotalReceivable']);
			}

			if (isset($data['movingUpTotalReceivable'])) {
				$this->updateReceivablesForAdjustment($params['studentID'], $params['schoolYearID'], 11, $data['movingUpTotalReceivable']);
			}
		...
		else
		{
			...
			// update the receivables 
			if (isset($data['cateringTotalReceivable'])) {
				$this->updateReceivablesForAdjustment($data['studentID'], $data['schoolYearID'], 1, $data['cateringTotalReceivable']);
			}

			if (isset($data['charityTotalReceivable'])) {
				$this->updateReceivablesForAdjustment($data['studentID'], $data['schoolYearID'], 2, $data['charityTotalReceivable']);
			}

			if (isset($data['extracurricularTotalReceivable'])) {
				$this->updateReceivablesForAdjustment($data['studentID'], $data['schoolYearID'], 3, $data['extracurricularTotalReceivable']);
			}

			if (isset($data['christmasTotalReceivable'])) {
				$this->updateReceivablesForAdjustment($data['studentID'], $data['schoolYearID'], 4, $data['christmasTotalReceivable']);
			}

			if (isset($data['familyDayTotalReceivable'])) {
				$this->updateReceivablesForAdjustment($data['studentID'], $data['schoolYearID'], 5, $data['familyDayTotalReceivable']);
			}

			if (isset($data['pictureTotalReceivable'])) {
				$this->updateReceivablesForAdjustment($data['studentID'], $data['schoolYearID'], 6, $data['pictureTotalReceivable']);
			}

			if (isset($data['gradFeeTotalReceivable'])) {
				$this->updateReceivablesForAdjustment($data['studentID'], $data['schoolYearID'], 7, $data['gradFeeTotalReceivable']);
			}

			if (isset($data['scoutingTotalReceivable'])) {
				$this->updateReceivablesForAdjustment($data['studentID'], $data['schoolYearID'], 8, $data['scoutingTotalReceivable']);
			}

			if (isset($data['othersTotalReceivable'])) {
				$this->updateReceivablesForAdjustment($data['studentID'], $data['schoolYearID'], 9, $data['othersTotalReceivable']);
			}

			if (isset($data['nutritionTotalReceivable'])) {
				$this->updateReceivablesForAdjustment($data['studentID'], $data['schoolYearID'], 10, $data['nutritionTotalReceivable']);
			}

			if (isset($data['movingUpTotalReceivable'])) {
				$this->updateReceivablesForAdjustment($data['studentID'], $data['schoolYearID'], 11, $data['movingUpTotalReceivable']);
			}
	...
	public function updateReceivablesForAdjustment($studentID, $schoolYearID, $categoryID, $newTotal)
    {
        // Step 1: Get all receivables for the student, year, and category
        $this->db->select('r.receivableID, r.receivableAmount');
        $this->db->from('receivables r');
        $this->db->join('batchreceivables b', 'r.batchReceivableID = b.batchReceivableID');
        $this->db->where('r.studentID', $studentID);
        $this->db->where('b.schoolYearID', $schoolYearID);
        $this->db->where('b.batchReceivableCategoryID', $categoryID);

        $receivables = $this->db->get()->result();

        if (empty($receivables)) {
            return false; // No receivables found
        }

        // Step 2: Compute total old amount and adjustment ratio
		$receivablesArray = json_decode(json_encode($receivables), true);
        $totalOld = array_sum(array_column($receivablesArray, 'receivableAmount'));

        if ($totalOld == 0) return false;

        $ratio = $newTotal / $totalOld;
        $remaining = $newTotal;


        // Step 3: Update each receivable proportionally
        foreach ($receivables as $index => $r) {
            $newAmount = round($r->receivableAmount * $ratio, 2);

            // Fix rounding for the last record
            if ($index === count($receivables) - 1) {
                $newAmount = $remaining;
            }

            $this->db->where('receivableID', $r->receivableID);
            $this->db->update('receivables', [
                'receivableAmount'   => $newAmount,
                'isManuallyAdjusted' => 1
            ]);
	...
	public function getLastReferenceNumber( $params ){
		...
		// ")->row();
		$row = $this->db->query("
			SELECT LPAD((refnum + 1), 6, '0') AS refnum
			FROM payments
			WHERE ref = {$params['ref']}
			ORDER BY paymentDate DESC, refnum DESC
			LIMIT 1
		")->row();

		return $row ? $row : (object)['refnum' => '000001'];
	}
	...
	public function getAgeGroup ( $gradeLevelID ){
		
		$this->db->where( 'gradelevelID', $gradeLevelID );
	...
	{
			$params['refnum'] = ltrim($params['refnum'], '0');
			...
			$this->db->where('s.studentID', $params['studentID']);
			$this->db->where('p.ref', $params['ref']);
			$this->db->where('p.refnum', $params['refnum']);
